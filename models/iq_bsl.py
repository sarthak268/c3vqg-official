...
class IQ(nn.Module):
    ...
    def __init__(self, vocab_size, max_len, hidden_size,
        ...
        # if self.category_space:
        #     self.category_embedding = nn.Embedding(num_categories, hidden_size / 8)
        #     self.category_encoder = MLP(hidden_size / 8, hidden_size / 4, hidden_size,
        #                        num_layers=num_layers)

        # Setup answer encoder.
        self.answer_encoder = EncoderRNN(vocab_size, max_len, hidden_size, num_categories,
        ...
        #                          rnn_cell=rnn_cell,
        #                          variable_lengths=True)
        # self.q_to_c = MLP(hidden_size, hidden_size, num_categories,
        #                        num_layers=num_layers)
        
        self.question_encoder = genQLSTM(hidden_size,hidden_size,vocab_size,num_categories,
                                        input_dropout_p=input_dropout_p,
                                        variable_lengths=True)    
        ...
        self.t_decoder = nn.Linear(z_size, hidden_size)
        ...
        #            z_size, att_ff_size, num_categories,
        #            num_layers=num_att_layers)
                # Setup category reconstruction.
        if self.category_space:
           self.category_reconstructor = MLP(
    ...
    def decode_questions_ours(self, image_features, ts,
                         questions=None, teacher_forcing_ratio=0,
                         decode_function=F.log_softmax):
        """Decodes the question from the latent space.
    ...
    def reconstruct_inputs_ours(self, image_features, category_features):
        """Reconstructs the image features using the VAE.
        ...
        # ts = self.reparameterize(t_mus, t_logvars)
        if self.image_recon:
    ...
    def encode_from_answer(self, images, answers, lengths=None):
        ...
        Args:
            ...
            categories: Batch of categories Tensors.
            alengths: List of answer lengths.
